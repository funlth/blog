...
class ArticleLike(models.Model):
    ...
    article = models.ForeignKey('ArticlePost', on_delete=models.CASCADE, related_name='likes')
...
'''
'''ArticlePost点赞模型关联通过likes_count() 方法相关联
 def likes_count(self):
        return self.articlelike_set.count()

    def like(self, user_id):
        like, created = ArticleLike.objects.get_or_create(user_id=user_id, article=self)
        if not created:
            like.delete()
        else:
            self.likes += 1
